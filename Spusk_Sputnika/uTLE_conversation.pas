...
begin
  ...
  MNMOTION := StrToFloat(Copy(TLE[1], 53, 11)); { Частота обращения (оборотов
    ...
    Нужна для вычисления a — большой полуоси орбиты (в километрах) }
  ...
  n := 2 * pi * MNMOTION; // среднее движение

  Elements[0] := Power(fm / Sqr(n), Third); { a — большая полуось орбиты
    (в километрах), перед расшифровкой необходимо домножить на вес в 1 / 3
    степени }
